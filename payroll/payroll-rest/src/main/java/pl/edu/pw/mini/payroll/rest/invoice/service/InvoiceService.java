...
public class InvoiceService {
    ...
    @Autowired
    private InvoiceDetailsDtoAssembler detailsDtoAssembler;

    @Autowired
    private AwsS3Service s3Service;

    public JsonListChunk<InvoiceDetailsDto> actualInvoices(JsonListRequest request) {
        ...
        return new JsonListChunk<>(
                detailsDtoAssembler.toDtoList(page.get()),
                page.getTotalElements(),
                page.getTotalPages() > request.getPageNumber() + 1
        );
    }

    public JsonListChunk<InvoiceDetailsDto> archivedInvoices(JsonListRequest request) {
    ...
    public InvoiceDetailsDto getInvoice(Long invoiceId) {
        ...
        if(Stream.of(InvoiceStatus.ACCEPTED, InvoiceStatus.PROCESSED).noneMatch(i -> i == invoice.getStatus())) {
            throw PAY_0006;
        }
        return detailsDtoAssembler.toDto(invoice);
    }
    public FileDto getInvoiceAttachment(Long invoiceId) throws IOException {
        Invoice invoice = repository.findById(invoiceId).orElseThrow(() -> PAY_0009);
        return FileDto
                .builder()
